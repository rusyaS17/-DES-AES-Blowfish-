...
static uint32_t static_keys[18] = {
...
creating_dynamic_keys()
{
    ...
    {
        dynamic_keys[kol] = static_keys[kol] ^ key8;
    }
    for (int i = 0; i <= 3; i++)
    {
        for (int j = 0; j <= 255; j++)
...
void blowfish()
{
    ...
    printf("encryption block: %lx\n",block_64);
    uint32_t tmp[18];
    for(int i = 0; i <= 17; i++)
    {
            tmp[i] = dynamic_keys[i];
    }
    for(int i = 0; i <= 17; i++)
    {
            dynamic_keys[i] = tmp[17 - i];
    }
    block_64 = fastly(block_64);
    printf("decryption block: %lx\n",block_64);
    return;
}